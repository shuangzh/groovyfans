		...
		handler: function() {
			...
			if(info.editIndex != undefined) {
				...
				if(rowchanges.length > 0) {
					...
					// 	console.info('get response status :' + status)
					// 	if(status == 'success') {
					...
					// 	} else {
					// 		console.info('handle failed response')
					...
					$.ajax({
                        ...
                        error:function(XMLHttpRequest, textStatus, errorThrown){
							console.info('handle failed response')
							$.messager.alert('ERROR', act + ' to server failed, server has exception. all changes rollback @@' + url +'  ' + XMLHttpRequest.responseText, 'error')
							$(info.sid).datagrid('rejectChanges')
                            info.editIndex = undefined
                            info.action = undefined
						}
